...
# 4. Calculate the installation of fiber optic cable by multiplying number of feet by a conditional statement
...
print(f"Welcome {user_name}! We aim to provide you the best fiber optic cables at the best price guaranteed!")
...
def get_amount_requested(prompt):
    ...
    :return: The valid value entered by user.
    """
    while True:
        try:
            ...
            print(f'The value entered is not a number. Please enter valid number for calculation.')
...
def cost_calculator(amount_requested, cost_per_foot= 0.95):
    """
    Provides the approved cost per foot of fiber optic cable.
    :param amount_requested: The amount in feet for fiber optic cable.
    :param cost_per_foot: The standard {0.95}/discount cost per foot depending on amount requested.
    :return: The total cost, the standard/discount applied.
    """
    try:
        if amount_requested <= 100:
            total_cost = amount_requested * cost_per_foot
            return total_cost, cost_per_foot
        elif 100 < amount_requested <= 250:
            cost_per_foot = 0.85
            total_cost = amount_requested * cost_per_foot
            return total_cost, cost_per_foot
        elif 250 < amount_requested <= 500:
            cost_per_foot = 0.75
            total_cost = amount_requested * cost_per_foot
            return total_cost, cost_per_foot
        else:
...
print(f"Thank you {user_name}!\n"
      f"Cost per foot is {locale.currency(cost_calculator(number_of_feet)[1])}\n"
      f"{company_name}'s total cost is {locale.currency(cost_calculator(number_of_feet)[0])} for {"%.2f" % number_of_feet} feet."
)
